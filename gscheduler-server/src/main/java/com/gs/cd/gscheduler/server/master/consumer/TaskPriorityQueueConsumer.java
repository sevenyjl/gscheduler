...
package com.gs.cd.gscheduler.server.master.consumer;
...
import com.gs.cd.gscheduler.common.task.sqoop.sources.SourceMysqlParameter;
import com.gs.cd.gscheduler.common.task.sqoop.targets.TargetMysqlParameter;
import com.gs.cd.gscheduler.common.thread.Stopper;
import com.gs.cd.gscheduler.server.entity.*;
import com.gs.cd.gscheduler.server.master.dispatch.ExecutorDispatcher;
import com.gs.cd.gscheduler.server.builder.TaskExecutionContextBuilder;
import com.gs.cd.gscheduler.server.entity.*;
import com.gs.cd.gscheduler.server.master.config.MasterConfig;
import com.gs.cd.gscheduler.server.master.dispatch.context.ExecutionContext;
import com.gs.cd.gscheduler.server.master.dispatch.enums.ExecutorType;
import com.gs.cd.gscheduler.server.master.dispatch.exceptions.ExecuteException;
import com.gs.cd.gscheduler.service.process.ProcessService;
import com.gs.cd.gscheduler.service.queue.TaskPriorityQueue;
...
//        taskInstance.getProcessInstance().setQueue(StringUtils.isEmpty(userQueue) ? tenant.getQueue() : userQueue);
        taskInstance.getProcessInstance().setQueue(userQueue);
