 ...
 */
package com.gs.cd.gscheduler.enums;
...
public enum Status {
    ...
    GSNOW_ERROR(10101, "GSnow Service error", "GSnow 服务请求异常"),
    ;
    ...
    public static Status valueOfWithCode(int code) {
        Status status = Arrays.stream(values()).filter(s -> s.getCode() == code).findFirst().orElse(null);
        if (status == null) {
            throw new RuntimeException("序列化status错误" + code);
        }
        return status;
    }
